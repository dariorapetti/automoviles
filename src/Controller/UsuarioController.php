...
class UsuarioController extends BaseController
{
    ...
    protected function getExtraParametersShowAction($entity): Array 
    {
        ...
        $query = "SELECT
                    ...
                    GROUP_CONCAT(CONCAT(CONVERT(s.sess_id,char), ':::', from_unixtime(s.sess_time, '%Y-%m-%d %H:%i:%s'), ':::', from_unixtime(s.sess_lifetime, '%Y-%m-%d %H:%i'), ':::', s.user_ip) ORDER BY s.sess_time DESC SEPARATOR '::::') AS sesiones
                FROM sessions s
                WHERE s.user_id = ?
                GROUP BY s.user_id";
